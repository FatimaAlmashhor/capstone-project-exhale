...
const SelfAssessment = ( ) =>{
  ...
  const handleBackStep = () => setDisplayIndex(pre => displayIndex === 0? pre : pre - 1 );
  const TasksBicker = Tasks.map((element ,index ) =>
      <div  key = {index} className = {`${display(index)}`}>
        <SelfAssessmentPanel  task ={element} onClick = {handleClick}/>
      </div>
      )
  return (
   <div className = 'flex flex-col mx-auto w-4/5 md:w-8/12 lg:w-6/12 mt-10'>
      <ProgressBar showLabel= {true} max={Tasks.length} now ={displayIndex}/>
      {TasksBicker}
      <div className='  mt-16  '>
        <button 
            type='button' 
            ...
            >
            <FontAwesomeIcon className= 'text-lg md:text-xl pt-1 text-green-400' icon = {['fas' , 'chevron-left']} />
